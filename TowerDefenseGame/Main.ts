namespace TowerDefenseGame{
	...
	import fudge = FudgeCore; // Core

	window.addEventListener("load", init);  // Start game

	// tslint:disable-next-line: indent
	let node: fudge.Node = new fudge.Node("Game");
	...
	function createBasicGround(){
		...
		let greenGround:fudge.Node = new fudge.Node("GreenGround");	// Create node - empty gameobject
		let boxMesh:fudge.MeshCube = new fudge.MeshCube();			// Create an empty body - mesh object
		let cmpBoxMesh:fudge.ComponentMesh = new fudge.ComponentMesh(boxMesh);	// Create an body component and add the mesh

		greenGround.addComponent(cmpBoxMesh);	// Add body component to the empty gameobject

		// Create Green Mat
		let matGreen: fudge.Material = new fudge.Material("GREEN", fudge.ShaderUniColor, new fudge.CoatColored(fudge.Color.CSS("GREEN")));	// Create material with a green color
		let cmpMatGreen: fudge.ComponentMaterial = new fudge.ComponentMaterial(matGreen);	// Create component for the material (so it can be added to the gameobject later)
	...
	function createWall(_toNode: fudge.Node, _scale: fudge.Vector3, _pos: fudge.Vector3){

		let wall: fudge.Node = new fudge.Node("Wall");
		let wallMesh: fudge.MeshCube = new fudge.MeshCube();
		let cmpWallMesh: fudge.ComponentMesh = new fudge.ComponentMesh(wallMesh);

		wall.addComponent(cmpWallMesh);

		let matGrey: fudge.Material = new fudge.Material("GREY", fudge.ShaderUniColor, new fudge.CoatColored(fudge.Color.CSS("GREY")));
		let cmpMatGrey: fudge.ComponentMaterial = new fudge.ComponentMaterial(matGrey);
...
*/
	...
	function first_scene():void{
		...
		let node: fudge.Node = new fudge.Node("Map");

		let baseMesh: fudge.MeshQuad = new fudge.MeshQuad();
		let baseMap: fudge.ComponentMesh = new fudge.ComponentMesh(baseMesh);
		node.addComponent(baseMap);

		let wall: fudge.ComponentMesh = new fudge.ComponentMesh(baseMesh);
		...
		let matGreen: fudge.Material = new fudge.Material("Green", fudge.ShaderUniColor, new fudge.CoatColored(fudge.Color.CSS("GREEN")));
		let cmpMatGreen: fudge.ComponentMaterial = new fudge.ComponentMaterial(matGreen);
	...
	function SetCamera(_xPos: number, _yPos: number, _zPos: number, _xRot: number, _yRot: number, _zRot: number):void{
	...
	function DrawBlackBox():void{

		// Create first node (Blue box)
		let boxNode: fudge.Node = new fudge.Node("BlackBox");
		let boxMesh: fudge.MeshQuad = new fudge.MeshQuad();
		let cmpBoxMesh: fudge.ComponentMesh = new fudge.ComponentMesh(boxMesh);

		boxNode.addComponent(cmpBoxMesh);	// Add mesh to the first box

		// Create Blue Mat
		let matBlue: fudge.Material = new fudge.Material("BLUE", fudge.ShaderUniColor, new fudge.CoatColored(fudge.Color.CSS("BLUE")));
		let cmpMatBlue: fudge.ComponentMaterial = new fudge.ComponentMaterial(matBlue);

		boxNode.addComponent(cmpMatBlue);	//Add mat to the first box

		// Create second node (Green box)
		let boxGreenNode: fudge.Node = new fudge.Node("GreenBox");
		let boxSmallMesh: fudge.MeshQuad = new fudge.MeshQuad();
		let cmpBoxMeshSmall: fudge.ComponentMesh = new fudge.ComponentMesh(boxSmallMesh);
		...
		let redColor: fudge.Color = new fudge.Color(0.9, 0.8, 0.7, 1);

		let newBox: fudge.Node = CreateBox("Box_1", redColor, new fudge.Vector3(0.5, 0, 0), new fudge.Vector3(0.2, 0.2, 0.2));
	...
	function CreateBox(_name: string, _color: fudge.Color, _pos: fudge.Vector3, _scale: fudge.Vector3): fudge.Node {
		let box: fudge.Node = new fudge.Node(_name);
		let boxMesh: fudge.Node = createCompleteMeshNode("Box", new fudge.Material("Box", fudge.ShaderUniColor, new fudge.CoatColored(_color)), new fudge.MeshCube);
		let cmpBoxMesh: fudge.ComponentMesh = boxMesh.getComponent(fudge.ComponentMesh);
